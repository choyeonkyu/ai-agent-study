...
# 1. 그래프 상태 정의 - 워크플로 전체에서 공유되는 데이터 구조
...
# 2. LangChain LLM 초기화 - 감정 분석에 사용할 AI 모델 설정
llm = ChatOpenAI(model="gpt-5-mini", max_tokens=300)

# 3. LLM기반 감정 분석 노드 - 첫번째 처리 단계
def analyze_emotion(state: EmotionBotState) -> Dict[str, Any]:
    message = state.user_message
    print(f"LLM 감정 분석 중: '{message}'")
    messages = [
        SystemMessage(content="다음 중 하나만 출력하세요 (다른 말은 절대 하지 마세요): positive, negative, neutral"),
...
def generate_neutral_response(state: EmotionBotState) -> Dict[str, Any]:
    responses = ["감사해요! 더 자세히 말씀해주세요.", 
                 "이해했어요. 다른 도움이 필요하면 말씀하세요!", 
                 "흥미로운 주제네요!"]
    return {"response": random.choice(responses)}

# 4. 조건부 라우팅 함수 - 감정 분석 결과에 따라 다음 노드 결정
...
def create_emotion_bot_graph():
    ...
    # 7. 시작 엣지 설정 - 워크플로의 진입점 정의
    workflow.add_edge(START, "analyze_emotion")

    # 8. 조건부 엣지 설정 - 동적 라우팅 구현
